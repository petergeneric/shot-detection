 ...
 */
public class UnbufferedCanvasFrame extends JFrame
{
	...
	public UnbufferedCanvasFrame(String title)
	{
		this(title, 0.0);
	}


	public UnbufferedCanvasFrame(String title, double gamma)
	{
		super(title);
	...
	public UnbufferedCanvasFrame(String title, GraphicsConfiguration gc)
	{
		this(title, gc, 0.0);
	}


	public UnbufferedCanvasFrame(String title, GraphicsConfiguration gc, double gamma)
	{
		super(title, gc);
		init(false, null, gamma);
	}
	...
	public UnbufferedCanvasFrame(String title, int screenNumber, DisplayMode displayMode) throws Exception
	{
		this(title, screenNumber, displayMode, 0.0);
	}


	public UnbufferedCanvasFrame(String title, int screenNumber, DisplayMode displayMode, double gamma) throws Exception
	{
	...
	private void init(final boolean fullScreen, final DisplayMode displayMode, final double gamma)
	{
		Runnable r = new Runnable()
		{
			public void run()
			{
				...
				System.out.println("GraphicsDevice.getDisplayMode: " + d);
				if (displayMode != null && d != null)
				{
					...
					d2 = new DisplayMode(w > 0 ? w : d.getWidth(),
					                     h > 0 ? h : d.getHeight(),
					                     b > 0 ? b : d.getBitDepth(),
					                     r > 0 ? r : d.getRefreshRate());
				}
				if (fullScreen)
				{
					setUndecorated(true);
					...
					setResizable(false);
					gd.setFullScreenWindow(UnbufferedCanvasFrame.this);
				}
				else
				{
	...
	private KeyEventDispatcher keyEventDispatch = new KeyEventDispatcher()
	{
		public boolean dispatchKeyEvent(KeyEvent e)
		{
			...
			{
				synchronized (UnbufferedCanvasFrame.this)
				{
					keyEvent = e;
					UnbufferedCanvasFrame.this.notify();
				}
			}
			return false;
		}
	...
	public Graphics2D createGraphics()
	{
		...
		{
			BufferedImage newbuffer = canvas.getGraphicsConfiguration().createCompatibleImage(canvas.getWidth(),
			                                                                                  canvas.getHeight(),
	...
	{
		showImage(image.getBufferedImage(image.getBufferedImageType() == BufferedImage.TYPE_CUSTOM ? 1.0 : inverseGamma,
		                                 flipChannels));
	}
